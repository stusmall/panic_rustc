...
// We could probably cache all the typedfuncs instead of the Instance to save a little load.  Lots
// of unsafeness here.  We need to be very careful about this copy from the sandbox.  Naughtiness
// can be afoot
fn run_wasm(instance: &Instance, v1: u32, v2: u32, data: &[u8]) {
    ...
    memory.read(result_struct.ptr as usize, &mut output_arrow_buffer).unwrap();
    ...
    let free = instance
        ...
        .unwrap();
    println!("Freeing input ptr 0x{:x?}", input_data_ptr);
    free.call((input_data_ptr, data.len() as i32, 0)).unwrap();
    println!("Freeing output arrow ptr 0x{:x?}", result_struct.ptr);
    free.call((result_struct.ptr, result_struct.len, 0)).unwrap();
    println!("Freeing return pointer 0x{:x?}", ret);
    free.call((ret, size_of::<UDFResult>() as i32, 0)).unwrap();
    println!(
        "Finished! UDF ran in {}ms",
        now.elapsed().as_millis()
    );
